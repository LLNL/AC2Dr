...
void material_effc(fdmesh *minfo, innout *finfo, int mrank, int nRank, MPI_Comm comm)
{
   ...
   double dr, dth, R0;
   double th, r, ths, rs;
   double rx0, rlen, rx, rz;
   double ceff0, rhoeff0, weff0;
   int effc, nx, ny;
   ...
   if(effc == 1) {
     ...
     for(i=0;i<nx;i++) {
         for(j=0;j<ny;j++) {
            ...
            //rth = atan(rz/rx);
            //vc = minfo->what[i][j] * cos(rth);
            vc = minfo->what[i][j] * rx0;
            //ceff0 = minfo->chat[i][j] + vc;
            ceff0 = minfo->chat[i][j] + vc;
            //Godin's definition
            rhoeff0 = minfo->rhohat[i][j]*pow(minfo->chat[i][j],2)/pow(ceff0,2);
            //Ostashev's definition
            //rhoeff0 = minfo->rhohat[i][j]*(1.0 - 2.0*vc/minfo->chat[i][j]);
            minfo->chat[i][j] = ceff0;
            minfo->rhohat[i][j] = rhoeff0;
            minfo->what[i][j] = weff0;
         }
      }
   }
...
/*
rx=seq(from=0,by=1,to=1000)*1000
rz=seq(from=0,by=1,to=80)*1000
rth=matrix(0,nrow=length(rx),ncol=length(rz))
