...
const char ver0[] = "0.9.9r4.2";
const char rdate[] = "December 2nd, 2025";
...
// Version number
    ...
    if( strcmp(argv[1], "-b2a" )==0) {
      int ftype;  //1d or 2d
      FILE *ptr_file0;
      double *bxx, *byy, **bzz, *bval;
      int bnx, bny;

      if(mrank==0) {
         printf("=============================================\n");
         printf("AC2Dr command (-b2a): file conversion from binary to ascii\n");
         printf("=============================================\n");

      ptr_file0 = fopen(argv[2], "rb");
      if(!ptr_file0) {
         sprintf(msg_buf, "     --- Error: The binary file (%s) does not exist.\n", argv[2]);
         elac_error(msg_buf, -1);
      }

      fread(&ftype, sizeof(int), 1, ptr_file0);
      fclose( ptr_file0 );

      if(ftype==1) {
         readbin_2col(argv[2], &byy, &bval, &bny);
         sprintf(msg_buf, "     The 1-d binary file (%s) is read.\n", argv[2]);
         printf(msg_buf);
         write_2col(argv[3], byy, bval, bny);
         sprintf(msg_buf, "     The 1-d ascii file (%s) is created.\n", argv[3]);
         printf(msg_buf);
      }

      if(ftype==2) {
         readbin_2dfile(argv[2], &bnx, &bny, &bxx, &byy, &bzz);
         sprintf(msg_buf, "     The 2-d binary file (%s) is read.\n", argv[2]);
         printf(msg_buf);
         writetxt_2dfile(argv[3], bnx, bny, bxx, byy, bzz);
         sprintf(msg_buf, "     The 2-d ascii file (%s) is created.\n", argv[3]);
         printf(msg_buf);
      }
      }
      exit(0);
    ...
    if( strcmp(argv[1], "-a2b" )==0) {
      int ftype, i0;  //1d or 2d
      FILE *ptr_file0;
      double *bxx, *byy, **bzz, *bval;
      int bnx, bny;
      char *pch0, buf0[1000], buf20[1000];

      if(mrank==0) {
         printf("=============================================\n");
         printf("AC2Dr command (-a2b): file conversion from ascii to binary\n");
         printf("=============================================\n");

      ptr_file0 = fopen(argv[2], "r");
      if(!ptr_file0) {
         sprintf(msg_buf, "     --- Error: The ascii file (%s) does not exist.\n", argv[2]);
         elac_error(msg_buf, -1);
      }

      fgets(buf0, 1000, ptr_file0);
      strcpy(buf20, buf0);
      i0=0;
      pch0 = strtok(buf20, " \n");
      while(pch0!=NULL) {
...
//         printf("pch0 = %s \n",pch0);
      ...
      printf("i0 = %d\n", i0);
      if(ftype==1) {
         read_2col(argv[2], &byy, &bval, &bny);
         sprintf(msg_buf, "     The 1-d ascii file (%s) is read.\n", argv[2]);
         printf(msg_buf);
         writebin_2col(argv[3], byy, bval, bny);
         sprintf(msg_buf, "     The 1-d binary file (%s) is created.\n", argv[3]);
         printf(msg_buf);
      }

      if(ftype==2) {
         read_2dfile(argv[2], &bnx, &bny, &bxx, &byy, &bzz);
         sprintf(msg_buf, "     The 2-d ascii file (%s) is read.\n", argv[2]);
         printf(msg_buf);
         writebin_2dfile(argv[3], bnx, bny, bxx, byy, bzz);
         sprintf(msg_buf, "     The 2-d binary file (%s) is created.\n", argv[3]);
         printf(msg_buf);
      }
      }
      exit(0);
...
// Read source
   ...
   MPI_Barrier(comm2);
   material_effc(&minfo, &finfo, mrank2, nRank, comm2);
//   MPI_Barrier(comm2);
//   nx = minfo.nx;
//   ny = minfo.ny;
//   for(j=0; j<ny; j++) {
...
//   mpi_printf(comm2, "MPI_test.", mrank2, nRank);
   ...
   //smoothing material models: velocity
   for(i=0; i<minfo.c_smooth; i++) {
   ...
   //smoothing material models: density
   for(i=0; i<minfo.rho_smooth; i++) {
   ...
   MPI_Barrier(comm2);
   rigidbound_material (minfo.rhohat, minfo.rhohat, minfo.what, minfo.nx, minfo.ny, mrank2, nRank, comm2);
   ...
   //smoothing material models: wind
   for(i=0; i<minfo.w_smooth; i++) {
   ...
   MPI_Barrier(comm2);
   rigidbound_material (minfo.what, minfo.what, minfo.what, minfo.nx, minfo.ny, mrank2, nRank, comm2);
