        ...
        if( pch != NULL && pch[0] != '#') {
            if( strcmp( pch, "mspeed" ) == 0 ) {
                ...
                while ( pch != NULL ) {
                    ...
                    if(strcmp(pch2,"smooth")==0) {
                        pch2=strtok(NULL,"=");
                        minfo->c_smooth = atoi(pch2);
                    }
                    itok++;
                    strcpy( buf2, buf );
                    pch = strtok( buf2, " \n" );
        ...
        if( pch != NULL && pch[0] != '#') {
            if( strcmp( pch, "wind" ) == 0 ) {
                ...
                while ( pch != NULL ) {
                    ...
                    if(strcmp(pch2,"effc")==0) {
                        pch2=strtok(NULL,"=");
                        //strcpy(*w_fn, pch2);
                        if(minfo->effc!=0)
                           elac_error("wind: effective sound speed approximation is already enabled.\n",-1);
                        minfo->effc = atoi(pch2);
                        //minfo->effc = 1;
                    }
                    if(strcmp(pch2,"format")==0) {
                        pch2=strtok(NULL,"=");
...
void get_time( char *fn, fdmesh *minfo, int mrank, int nRank, MPI_Comm comm2 )
{
    ...
    print_message_mpi(minfo->logf, comm2, buf, 200, mrank, nRank, 0);
...
//store all source information locally
    local_id = id_num;
...
//    minfo->sta_num = id_num;
    minfo->stas0 = (station *) malloc( id_num*sizeof(station) );
        ...
        if( pch != NULL && pch[0] != '#') {
            if( strcmp( pch, "rec" ) == 0 ) {
                ...
                while ( pch != NULL ) {
                    ...
                    minfo->stas0[id_num-1].x=x_g;
                    minfo->stas0[id_num-1].xi_global=xi_g;
                    minfo->stas0[id_num-1].y=sta_y;
                    minfo->stas0[id_num-1].yi=floor(sta_y/minfo->dr)+ghostL;
                    strcpy(minfo->stas0[id_num-1].mode,sta_mode);
                    strcpy(minfo->stas0[id_num-1].name,sta_name);
                    minfo->stas0[id_num-1].format=sta_format;
            }
        }
    }
        ...
        if( pch != NULL && pch[0] != '#') {
            if( strcmp( pch, "image" ) == 0 ) {
                ...
                while ( pch != NULL ) {
                    ...
                    if(strcmp(pch2,"ghostL")==0) {
                        pch2=strtok(NULL,"=");
                        //finfo->imgs[id_num-1].format=atoi(pch2);
                        finfo->imgs[id_num-1].ghostL=0;
                        if(strcmp(pch2,"0")==0) finfo->imgs[id_num-1].ghostL = 0;
                        if(strcmp(pch2,"1")==0) finfo->imgs[id_num-1].ghostL = 1;
                    }
                    itok++;
                    strcpy( buf2, buf );
                    pch = strtok( buf2, " \n" );
...
void print_message (char *fn, char *msg, int NEW) {
    FILE *ptr_file;
    if(NEW) {
      ptr_file=fopen(fn, "w");
    } else {
...
void print_messagef (char *fn, char *msg, int NEW) {
    FILE *ptr_file;
    if(NEW) {
      ptr_file=fopen(fn, "w");
    } else {
