...
void decompose_domain (fdmesh *minfo, innout *finfo, int mrank, int totalp, int *nRank, MPI_Comm comm)
{
...
//   xi_start=(int *) malloc (10 * sizeof(int));
//   xi_end=(int *) malloc (10 * sizeof(int));
//   nx_local=(int *) malloc (10 * sizeof(int));


//   sprintf(buf, "totalp = %d", totalp);
//   mpi_printf(comm, buf, mrank, totalp);
   ghost0 = ghostL;
   nx_tmp = ceil((double) (minfo->nx_global-ghost0) / (double) totalp);
   if(nx_tmp<(ghostL+1)) nx_tmp=ghostL+1;
...
//   printf("Process %d/%d: nx_global = %ld...\n",mrank, totalp, minfo->nth_global);
   ...
   while(xi_end[i] < ((minfo->nx_global-ghost0) -1))
   {
      i=i+1;
      xi_start[i] = nx_tmp*i;
      xi_end[i] = (nx_tmp*(i+1))-1;
   ...
   xi_end[i] = (minfo->nx_global-ghost0) - 1;
   nx_local[i] = xi_end[i] - xi_start[i] + 1;
   if(nx_local[i] >= 4) {
      *nRank = i;
   } else {
...
//   printf("Process %d/%d: nx_local = %d...\n",mrank, totalp, nx_local[mrank]);
   ...
   if(mrank == 0) {
      minfo->xi_start = xi_start[mrank];
      minfo->xi_end = xi_end[mrank]+ghost0+ghost0;
      minfo->nth = nx_local[mrank]+ghost0+ghost0;
      minfo->nx = nx_local[mrank]+ghost0+ghost0;
   }

   if(mrank == *nRank) {
      minfo->xi_start = xi_start[mrank]-ghost0+ghost0;
      minfo->xi_end = xi_end[mrank]+ghost0;
      minfo->nth = nx_local[mrank]+ghost0;
      minfo->nx = nx_local[mrank]+ghost0;
   }

   if((mrank > 0) & (mrank < *nRank)) {
      minfo->xi_start = xi_start[mrank]-ghost0+ghost0;
      minfo->xi_end = xi_end[mrank]+ghost0+ghost0;
      minfo->nth = nx_local[mrank]+2*ghost0;
      minfo->nx = nx_local[mrank]+2*ghost0;
   }
