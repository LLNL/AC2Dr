...
#R library for AC2Dr
#v0.9.9r3.2
#Changes from v0.9.9r3.2
#  1. add functions to read/write 2-d material files in ASCII format (readtxt.2dmat, maketxt.2dmat)

### Read a 2d-section image (binary)
...
### Read a 2-d material file (ASCII)
readtxt.2dmat <- function (fn) {
#z is a m x n matrix, z[m][n] = z[x(1, ..., m)][y(1, ..., n)].
#x is angle in degree, y is elevation in meter.
    aa0=scan(fn, what=0.0)
    ftype = aa0[1]
    nx1= aa0[2]
    ny1= aa0[3]
    dx1= aa0[4]
    dy1= aa0[5]
    bb1=matrix(aa0[6:length(aa0)],nrow=nx1,ncol=ny1,byrow=TRUE)
...
### Write a 2-d material file (ASCII)
maketxt.2dmat <- function (x, y, z, fn, swap=0) {
#z is a m x n matrix, z[m][n] = z[x(1, ..., m)][y(1, ..., n)].
#x is angle in degree, y is elevation in meter.
   ftype=2
   dx = diff(x)[1]
   ...
   cat(sprintf("%d\n", ftype), file=fn, append=FALSE)
   cat(sprintf("%d\n", nx), file=fn, append=TRUE)
   cat(sprintf("%d\n", ny), file=fn, append=TRUE)
   cat(sprintf("%e\n", dx), file=fn, append=TRUE)
   cat(sprintf("%e\n", dy), file=fn, append=TRUE)

   for(i in 1:nx) {
      for(j in 1:ny) {
         cat(sprintf("%f\n", z[i,j]), file=fn, append=TRUE)
      }
   }
   close(fin)
}
...
#z is a vector, z[y], of elevation y (meter).
   aa=scan(fn, what=list(y=0.0,z=0.0))
   return(aa)
}
