...
//2-D weather model
   if(c_type==3) {
      ...
      material_intp_2d(nx0, ny0, xx0, yy0, zz0, minfo->nx, minfo->ny, minfo->xx, minfo->yy, minfo->chat);
   ...
   if(rho_type==3) {
      ...
      material_intp_2d(nx0, ny0, xx0, yy0, zz0, minfo->nx, minfo->ny, minfo->xx, minfo->yy, minfo->rhohat);
   ...
   if(w_type==3) {
      ...
      material_intp_2d(nx0, ny0, xx0, yy0, zz0, minfo->nx, minfo->ny, minfo->xx, minfo->yy, minfo->what);
...
void material_minmax(fdmesh *minfo, int mrank, int nRank, MPI_Comm comm2) {
   ...
   char buf[200];
    ...
    whmin = minfo->what[0][0];
    whmax = minfo->what[0][0];
    cmin = minfo->chat[0][0];
    cmax = minfo->chat[0][0];
    rhomin = minfo->rhohat[0][0];
    rhomax = minfo->rhohat[0][0];

    for(i=0; i<(minfo->nth*minfo->nr); i++) {
       if(minfo->what[0][i] < whmin) { whmin = minfo->what[0][i]; }
       if(minfo->what[0][i] > whmax) { whmax = minfo->what[0][i]; }
       if(minfo->rhohat[0][i] < rhomin) { rhomin = minfo->rhohat[0][i]; }
       if(minfo->rhohat[0][i] > rhomax) { rhomax = minfo->rhohat[0][i]; }
       if(minfo->chat[0][i] < cmin) { cmin = minfo->chat[0][i]; }
       if(minfo->chat[0][i] > cmax) { cmax = minfo->chat[0][i]; }
    }
