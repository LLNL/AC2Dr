...
void print_message_mpi (char *fn, MPI_Comm comm, char *msg, int nc, int mrank, int nRank, int NEW ) {
   int i;
   char buf0[500], buf1[1000];
   MPI_Status status;

   if(comm != MPI_COMM_NULL) {
      if(mrank==0) {
         sprintf(buf1, "Process %d/%d: %s\n", mrank, nRank, msg);
         print_message(fn, buf1, NEW);
      }
...
void print_messagef_mpi (char *fn, MPI_Comm comm, char *msg, int nc, int mrank, int nRank, int NEW ) {
   int i;
   char buf0[500], buf1[1000];
   MPI_Status status;
...
//         printf("Process %d/%d: nx1=%d, ny1=%d Received from %d\n", mrank, nRank, nx1, ny1, cRank);   
      ...
      fwrite(&imgt, sizeof(double), 1, ptr_file);
   }
...
void wdbin_mpi_2dimg_ngl( char *fn, double **m, int f1, double imgt, \
   ...
   FILE *ptr_file;
   int i, j, cRank;
   int nx_global1, ny_global1;
   int tag1=1, tag2=2, tag3=3;
   MPI_Status status;

   nx1 = nx; ny1 = ny;
   dx0 = dx; dy0 = dy;
   nx_global1 = nx_global-2*ghostL;
   ny_global1 = ny_global-2*ghostL;
   ...
   if(mrank == 0) {
      ...
      ptr_file = fopen(fn, "wb");
      fwrite(&f1, sizeof(int), 1, ptr_file);
      fwrite(&nx_global1, sizeof(int), 1, ptr_file);
      fwrite(&ny_global1, sizeof(int), 1, ptr_file);
      fwrite(&dx0, sizeof(double), 1, ptr_file);
      fwrite(&dy0, sizeof(double), 1, ptr_file);
      for(i=ghostL; i < (nx1-ghostL); i++) 
      {
         for(j=ghostL; j< (ny1-ghostL); j++)
         {
            fwrite(&m[i][j], sizeof(double), 1, ptr_file);
         }
      }
   ...
   MPI_Barrier(comm);
   //printf("Process2 %d/%d: nx1=%d, ny1=%d Set\n", mrank, nRank, nx1, ny1);   
